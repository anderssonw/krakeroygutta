...
<div class="structure">
	<div class="w-full flex justify-between">
		<FilterMenu
			...
			bind:selectedOption={seasonOption}
		...
		<div class="w-full flex flex-col gap-8">
			...
			<div class="grid gap-4">
				{#each sortByFilter(allPlayerStatistics, filterOption) as player}
					<PlayerStatisticsTable {player} />
				{/each}
			</div>

			<PlayerFantasyRanks {fantasyTeamPlayers} {allPlayerStatistics} />
		</div>
	{:else if seasonOption.id === seasonDefaultOptions[1].id}
		<div class="w-full flex flex-col gap-16">
			{#each seasons.sort((a, b) => b.id - a.id) as seas}
				<div class="w-full flex flex-col gap-8">
					...
					<PlayerFantasyRanks fantasyTeamPlayers={fantasyTeamPlayers.filter((ftp) => ftp.season_id === seas.id)} {allPlayerStatistics} />
				</div>
			{/each}
		</div>
	{:else if allPlayerStatistics.length > 0}
		<div class="w-full flex flex-col gap-8">
			<div class="flex justify-center px-2">
				<h1 class="text-center text-3xl tablet:text-4xl">{seasonOption.name}</h1>
			</div>
			...
			<PlayerFantasyRanks
				fantasyTeamPlayers={fantasyTeamPlayers.filter((ftp) => ftp.season_id === seasonOption.id)}
				{allPlayerStatistics}
			/>
		</div>
	{:else}
