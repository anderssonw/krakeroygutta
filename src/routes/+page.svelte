	...
	{#if season}
		<div class="relative flex w-full justify-center sm:w-sm">
			...
			<div class="absolute flex flex-col gap-4 px-8 pt-8 text-sm sm:text-base">
				...
				<span>
					Den {season.deadline_time.toLocaleDateString('no-NO', { dateStyle: 'long' })} braker det løs, både i hallen på Kvernhuset og Casa Bjerkholt.
					Klokken {season.deadline_time.toLocaleTimeString('no-NO', { timeStyle: 'short' })} samme dag vil det ikke lenger være mulig å endre
					på laget sitt.
				</span>
