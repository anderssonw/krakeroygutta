				...
				{#if season && !isSeasonPastDeadline(season)}
					<form method="POST" class="flex justify-center">
						{#if bet.better?.id == user?.id}
							<button type="submit" class="btn-dark" formaction="?/removeBet"> Fjern veddemål </button>
						{:else}
							<button type="submit" class="btn-dark" formaction={placedBet(bet, user?.id ?? '') ? '?/removeBetAgainst' : '?/addBetAgainst'}>
