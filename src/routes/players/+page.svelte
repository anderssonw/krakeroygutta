...
<div class="structure">
	...
	{:then resolvedPlayers}
		{#if resolvedPlayers && sortPlayers(resolvedPlayers).length > 0}
			<Players players={sortPlayers(resolvedPlayers)} {season} />
		{:else}
		...
		{/if}
	{:catch error}
		<p>Feil ved lasting av spillere: {error.message}</p>
	{/await}
</div>
