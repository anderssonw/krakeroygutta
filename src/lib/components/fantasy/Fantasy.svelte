...
<div class="structure">
    ...
    {:else}
        ...
        <div class="relative w-full h-96 bg-primary-color">
            <div class="w-20 h-20 rounded-full bg-primary-color-light absolute bottom-0 left-1/2 -translate-x-1/2 overflow-hidden">
                {#if myPlayers[0]}
                    <img src={myPlayers[0].Picture} alt="head">
                {/if}
            </div>
            <div class="w-20 h-20 rounded-full bg-primary-color-light absolute top-1/2 left-0 -translate-y-1/2 overflow-hidden">
                {#if myPlayers[1]}
                    <img src={myPlayers[1].Picture} alt="head">
                {/if}
            </div>
            <div class="w-20 h-20 rounded-full bg-primary-color-light absolute top-1/2 right-0 -translate-y-1/2 overflow-hidden">
                {#if myPlayers[2]}
                    <img src={myPlayers[2].Picture} alt="head">
                {/if}
            </div>
            <div class="w-20 h-20 rounded-full bg-primary-color-light absolute top-0 left-1/2 -translate-x-1/2 overflow-hidden">
                {#if myPlayers[3]}
